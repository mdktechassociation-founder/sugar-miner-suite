...
class MiningSession {
  ...
  Future<void> run() async {
    ...
    while (running) {
      ...
      // Stay inside the CPU budget the user agreed to. Sleeping here also lets
      // the phone's scheduler run everything else it needs to.
      if (dutyShare < 1.0) {
        final workedMs = DateTime.now().difference(batchStart).inMilliseconds;
        final sleepMs = (workedMs * (1 - dutyShare) / dutyShare).round();
        if (sleepMs > 0) {
          await Future<void>.delayed(Duration(milliseconds: sleepMs.clamp(1, 4000)));
        }
      }
