/// SHA-256, in Dart, with no dependencies.
///
/// Kept here rather than pulled from pub.dev so this package has none: a wallet
/// package that resolves to nothing cannot break because a transitive dependency
/// changed, and it can be tested on any machine with just the Dart SDK.
library;
...
class Sha256 {
  ...
  static int _rr(int x, int n) => ((x >>> n) | (x << (32 - n))) & 0xffffffff;
  ...
  static List<int> digest(List<int> input) {
    ...
    for (var block = 0; block < padded.length; block += 64) {
      ...
      for (var t = 16; t < 64; t++) {
        final s0 = _rr(w[t - 15], 7) ^ _rr(w[t - 15], 18) ^ (w[t - 15] >>> 3);
        final s1 = _rr(w[t - 2], 17) ^ _rr(w[t - 2], 19) ^ (w[t - 2] >>> 10);
