/// BIP-39: the word phrase a person can write down, and the seed it stands for.
///
/// Why this exists at all: a raw private key is unusable as a backup — nobody
/// writes 64 hex characters down correctly, and nobody can read them back. Twelve
/// words are checkable by eye (the last one is a checksum) and restorable in any
/// BIP-39 wallet. Everything else in this package still works on raw keys; this is
/// the layer that makes a key worth trusting to paper.
///
/// English words only. BIP-39 says the phrase and passphrase are NFKD-normalised
/// before the seed is computed, and this package has no Unicode normalisation —
/// for the 2048 English words, all ASCII, normalisation is the identity, and an
/// ASCII passphrase is too. A non-ASCII passphrase would not match another
/// implementation, so [seedOf] rejects one rather than producing a wallet that
...
class Mnemonic {
  /// 128 bits → 12 words, 256 bits → 24 words. The entropy must be a multiple of
  /// 32 bits; 128 and 256 are the ones every wallet agrees on.
  static const int strength12 = 128;
  static const int strength24 = 256;

  /// The 2048 words, packed sixteen to a line.
  ///
  /// Each line ends in a space and the adjacent literals concatenate, so this is
  /// one long string; it is split once below. The guard exists because getting
  /// this wrong is silent: a missing space merges two words, the list comes out
  /// short, and every phrase is wrong in a way no checksum notices.
  ...
  static final List<String> words = () {
    final split = _packed.trim().split(' ');
    if (split.length != 2048) {
      throw StateError('the BIP-39 word list has ${split.length} entries, not 2048');
    }
    return List<String>.unmodifiable(split);
  }();
  ...
  /// Turns entropy into a phrase: the entropy in 11-bit groups, with a checksum
  /// of the first (entropy bits ÷ 32) bits of its SHA-256 appended.
  static String fromEntropy(List<int> entropy) {
    if (entropy.length != 16 && entropy.length != 32) {
      throw ArgumentError('entropy is 16 or 32 bytes, got ${entropy.length}');
    }
    ...
    // the checksum bytes are already inside the entropy's own hash
    final hash = _Sha256Digest.of(entropy);
    for (var i = 0; i < checksumBits; i++) {
      final bit = (hash[i ~/ 8] >> (7 - (i % 8))) & 1;
      bits.write(bit);
    }
    final all = bits.toString();
    if (all.length % 11 != 0) {
      throw StateError('internal: bit length ${all.length} is not a multiple of 11');
    }
    final out = <String>[];
    for (var i = 0; i < all.length; i += 11) {
      out.add(words[int.parse(all.substring(i, i + 11), radix: 2)]);
    }
    return out.join(' ');
  }

  /// True when every word is in the list and the checksum matches — which is what
  /// catches the commonest restore mistake, a mistyped or swapped word.
  static bool isValid(String phrase) {
    final parts = normalise(phrase).split(' ');
    ...
    final all = bits.toString();
    final entropyBits = (parts.length == 12) ? 128 : 256;
    final checksumBits = entropyBits ~/ 32;
    if (all.length != entropyBits + checksumBits) return false;

    final ent = <int>[];
    for (var i = 0; i + 8 <= entropyBits; i += 8) {
      ent.add(int.parse(all.substring(i, i + 8), radix: 2));
    }
    final hash = _Sha256Digest.of(ent);
    for (var i = 0; i < checksumBits; i++) {
      final want = (hash[i ~/ 8] >> (7 - (i % 8))) & 1;
      final got = all.codeUnitAt(entropyBits + i) - 0x30;
      if (want != got) return false;
    }
    return true;
  }

  /// The entropy a phrase stands for, or null when the phrase is not valid.
  static Uint8List? entropyOf(String phrase) {
    if (!isValid(phrase)) return null;
    final parts = normalise(phrase).split(' ');
    var bits = StringBuffer();
    for (final w in parts) {
      bits.write(_index[w]!.toRadixString(2).padLeft(11, '0'));
    }
    final all = bits.toString();
    final entropyBits = (parts.length == 12) ? 128 : 256;
    final out = Uint8List(entropyBits ~/ 8);
    for (var i = 0; i < out.length; i++) {
      out[i] = int.parse(all.substring(i * 8, i * 8 + 8), radix: 2);
    }
    return out;
  }

  /// The 64-byte seed BIP-32 starts from: PBKDF2-HMAC-SHA512 over the phrase with
  /// the string "mnemonic" as the salt prefix, 2048 rounds.
  static Uint8List seedOf(String phrase, {String passphrase = ''}) {
    final normalised = normalise(phrase);
    if (!isValid(normalised)) {
      throw const FormatException('that is not a valid 12- or 24-word phrase');
    }
    for (final unit in (normalised + passphrase).codeUnits) {
      if (unit > 0x7f) {
        throw const FormatException(
            'only English phrases and ASCII passphrases are supported: this package '
            'cannot normalise Unicode, and a seed computed without it would not '
            'restore in any other wallet');
      }
    }
    return Pbkdf2.derive(
      password: normalised.codeUnits,
      salt: ('mnemonic$passphrase').codeUnits,
      iterations: 2048,
      length: 64,
    );
  }

  /// Generates a phrase from the platform's secure random source.
  static String generate({
    required Uint8List Function(int) random,
    int strength = strength12,
  }) {
    if (strength != strength12 && strength != strength24) {
      throw ArgumentError('strength is 128 or 256 bits');
    }
    return fromEntropy(random(strength ~/ 8));
  }

  /// Trims, lowercases and collapses internal whitespace, which is how phrases
  /// arrive from a paste, a photo of paper, or a person retyping them.
  static String normalise(String phrase) =>
      phrase.trim().toLowerCase().split(RegExp(r'\s+')).join(' ');
}

/// SHA-256 lives in sha256.dart; this alias keeps the import list here short and
/// makes it obvious that the checksum is SHA-256 (BIP-39), not SHA-512.
class _Sha256Digest {
  static List<int> of(List<int> data) => Sha256.digest(data);
}
