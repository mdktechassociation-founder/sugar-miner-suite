...
| **Chrome / Chromium (web)** | ✅ CI, and now **run** in a real browser | `tools/verify_chrome.js` serves the published page over HTTP and opens it in real headless Chrome: both of the page's engines (SIMD and scalar) must reproduce the genesis PoW hash *there*, the page's own `sha256d` must reproduce the genesis block hash, the payout field must still ship empty, the bridge field must never name a third party, and nothing may be logged as an error | ✅ the page loads and its engines run (verified on Chrome 154; CI runs it on every push with `--strict`, which fails if it had to skip) | ❌ a browser cannot open a TCP socket; the wallet build refuses to mine in one plain sentence, and `sugar-miner/` is the web answer through a WebSocket bridge |
...
| the published page | `browser miner — the page and the bridge behave` (Node) and `browser miner — the published page in a real browser` (Chrome) |
